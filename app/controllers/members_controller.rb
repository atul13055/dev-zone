class MembersController < ApplicationController
  ...
  def update_skill
     user_skills = params.dig(:user, :skills) || [] 
    if current_user.update(skills: user_skills.reject(&:empty?))
      render_turbo_stream(
        'replace',
        'member-skill',
        'members/member_skill',
        { user: current_user }
      )
    end
  end
  ...
  def user_personal_info_params
    params.require(:user).permit(:first_name, :last_name, :city, :state, :country, :pincode, :profile_title, :contact_number, :street_address, :image,skills: [])
  end
